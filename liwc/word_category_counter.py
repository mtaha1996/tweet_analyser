...
_liwc_tokenizer = re.compile(r'(\d[^a-z\(\)]*|[a-z](?:[\'\.]?[a-z])*|(?<=[a-z])[^a-z0-9\s\(\)]+|[\(\)][^a-z]*)',
...
class Dictionary():
    ...
    def __init__(self, filename, use_long_category_names=True,
                 internal_category_list=None):  ##################################
                    ...
                    for entry_type in self._lookup[word]:
                        if entry_type == Dictionary._TYPE_BASIC:
                            ...
                            for conditions, if_set, if_not_set in self._lookup[word][entry_type]:
                                if ((entry_type == Dictionary._TYPE_PRE and not set(
                                        self.score_word(word=previous_word, next_word=word).keys()).isdisjoint(
                                        set(conditions))) or
                                        (
                        ...
                        if char in current_node:
                            ...
                            if isinstance(current_node, set):
                                ...
                                self._lookup[word][Dictionary._TYPE_BASIC].update(
                                    current_node)  # add to doc2vec lookup for time efficiency
